...
public class MessageReader(Stream stream)
{
    ...
    public void ReadSuccessBytes()
    {
        ...
        foreach (var b in buffer)
        {
            if (b == 255)
                continue;
            Console.Error.Write(
                $"Catastrophic error. Did not find success bytes. Instead found: [{string.Join(", ", buffer)}]"
            );
            Environment.Exit(1);
        }
    }
    ...
    private void ReadStdIn(byte[] buffer, int count)
    {
        ...
        while (true)
        {
            var bytesReadCount = stream.Read(buffer, offset, count - offset);
...
public class MessageWriter(Stream stream)
{
