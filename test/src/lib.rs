...
mod test {
    ...
    async fn general_tests() {
        ...
        for _ in 0..100 {
            ...
            assert_eq!(plugin_info.name, "dprint-plugin-csharpier");
            ...
            assert_eq!(
                file_matching_info.file_extensions,
        ...
        for _ in 0..1000 {
            ...
            handles.push(dprint_core::async_runtime::spawn({
                ...
                async move {
                    ...
                    let result = communicator
                        .format_text(ProcessPluginCommunicatorFormatRequest {
                            file_path: PathBuf::from("file.csx"),
                            file_text: "namespace Test    {   }\n".to_string(),
                            range: None,
                            config_id,
                            override_config: Default::default(),
        ...
        communicator
            .check_config_updates(Default::default())
            .await
            .unwrap();
    ...
    async fn new_communicator() -> ProcessPluginCommunicator {
        let exe_path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("../DprintPluginCsharpier/bin/Debug/net8.0/")
            .join(if cfg!(windows) {
                "dprint-plugin-csharpier.exe"
            } else {
                "dprint-plugin-csharpier"
            });
        ProcessPluginCommunicator::new(&exe_path, |err| eprintln!("{}", err))
            .await
            .unwrap()
    }
}
